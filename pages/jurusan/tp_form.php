...
$PAGE->set_url(new moodle_url('/local/akademikmonitor/pages/jurusan/tp_form.php', ['id' => $id, 'kmid' => $kmid, 'cpid' => $cpid]));
$PAGE->set_context($context);
$PAGE->set_title($id ? 'Edit Tujuan Pembelajaran' : 'Tambah Tujuan Pembelajaran');
$PAGE->set_heading($id ? 'Edit Tujuan Pembelajaran' : 'Tambah Tujuan Pembelajaran');
...
$record = $id ? $DB->get_record('tujuan_pembelajaran', ['id' => $id, 'id_capaian_pembelajaran' => $cpid], '*', MUST_EXIST) : null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($deskripsi === '') {
        redirect(new moodle_url('/local/akademikmonitor/pages/jurusan/tp_form.php', ['id' => $postid, 'kmid' => $kmid, 'cpid' => $cpid]), 'Deskripsi TP wajib diisi.', null, \core\output\notification::NOTIFY_ERROR);
    }

    $data = new stdClass();
    if ($hasKonten) {
        $data->konten = substr(trim(optional_param('konten', '', PARAM_TEXT)), 0, 100);
    }
    $data->kompetensi = trim(optional_param('kompetensi', '', PARAM_TEXT));
    $data->dpl = trim(optional_param('dpl', '', PARAM_TEXT));
    $data->atp = trim(optional_param('atp', '', PARAM_TEXT));
    $data->deskripsi = $deskripsi;
    $data->id_capaian_pembelajaran = $cpid;

    if ($postid > 0) {
        $data->id = $postid;
        $DB->update_record('tujuan_pembelajaran', $data);
        \local_akademikmonitor\service\tp_gradebook_service::sync_grade_item_name((int)$postid);
        redirect(new moodle_url('/local/akademikmonitor/pages/jurusan/tujuan_pembelajaran.php', ['kmid' => $kmid, 'cpid' => $cpid]), 'Tujuan pembelajaran berhasil diperbarui.', null, \core\output\notification::NOTIFY_SUCCESS);
    }
    ...
    redirect(new moodle_url('/local/akademikmonitor/pages/jurusan/tujuan_pembelajaran.php', ['kmid' => $kmid, 'cpid' => $cpid]), 'Tujuan pembelajaran berhasil ditambahkan.', null, \core\output\notification::NOTIFY_SUCCESS);
}
...
$templatecontext = array_merge(local_akademikmonitor_backend_admin_urls('jurusan'), [
    ...
    'konten' => $record && $hasKonten ? s((string)($record->konten ?? '')) : '',
    'kompetensi' => $record ? s((string)($record->kompetensi ?? '')) : '',
    'dpl' => $record ? s((string)($record->dpl ?? '')) : '',
    'atp' => $record ? s((string)($record->atp ?? '')) : '',
    'deskripsi' => $record ? s((string)$record->deskripsi) : '',
    'is_edit' => (bool)$record,
    'action_url' => (new moodle_url('/local/akademikmonitor/pages/jurusan/tp_form.php', ['id' => $id, 'kmid' => $kmid, 'cpid' => $cpid]))->out(false),
    'cancel_url' => (new moodle_url('/local/akademikmonitor/pages/jurusan/tujuan_pembelajaran.php', ['kmid' => $kmid, 'cpid' => $cpid]))->out(false),
    'sesskey' => sesskey(),
]);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_akademikmonitor/tp_form', $templatecontext);
echo $OUTPUT->footer();
