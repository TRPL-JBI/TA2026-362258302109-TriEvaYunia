...
$PAGE->set_url(new moodle_url('/local/akademikmonitor/pages/jurusan/setdetail.php', ['id' => $id]));
$PAGE->set_context($context);
$PAGE->set_title('Set Detail Mata Pelajaran');
$PAGE->set_heading('Set Detail Mata Pelajaran');
...
$km = $DB->get_record('kurikulum_mapel', ['id' => $id], '*', MUST_EXIST);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $jam = local_akademikmonitor_backend_jam(optional_param('jam', '', PARAM_TEXT));
    $kktp = max(0, min(100, optional_param('kktp', 0, PARAM_INT)));

    $km->tingkat_kelas = $tingkat;
    $km->jam_pelajaran = $jam;
    $km->kktp = $kktp;
    $DB->update_record('kurikulum_mapel', $km);

    redirect(new moodle_url('/local/akademikmonitor/pages/jurusan/setmapel.php', ['id' => $kj->id_jurusan]), 'Detail mata pelajaran berhasil disimpan.', null, \core\output\notification::NOTIFY_SUCCESS);
}

$templatecontext = array_merge(local_akademikmonitor_backend_admin_urls('jurusan'), [
    'nama' => format_string($mapel->nama_mapel),
    'tingkat_x' => $km->tingkat_kelas === 'X',
    'tingkat_xi' => $km->tingkat_kelas === 'XI',
    'tingkat_xii' => $km->tingkat_kelas === 'XII',
    'jam' => s((string)($km->jam_pelajaran ?? '')),
    'kktp' => $km->kktp !== null ? (int)$km->kktp : 0,
    'action_url' => (new moodle_url('/local/akademikmonitor/pages/jurusan/setdetail.php', ['id' => $id]))->out(false),
    'back_url' => (new moodle_url('/local/akademikmonitor/pages/jurusan/setmapel.php', ['id' => $kj->id_jurusan]))->out(false),
    'sesskey' => sesskey(),
]);

echo $OUTPUT->header();
echo $OUTPUT->render_from_template('local_akademikmonitor/setdetail', $templatecontext);
echo $OUTPUT->footer();
